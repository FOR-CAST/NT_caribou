...
mergePredictedCaribouTables <- function(pathToFiles, ## TODO: unused argument
                                        climateScenarios,
                                        yr,
                                        runs,
                                        studyAreaName,
                                        herdSHP) { ## TODO: unused argument
  allTbs <- grepMulti(list.files(dirname(Paths$outputPath),
      recursive = TRUE,
      full.names = TRUE
    ),
    patterns = c(
      "predictedCaribou_Year",
  ...
  DTcs <- rbindlist(lapply(climateScenarios, function(eachClim) {
    DTr <- rbindlist(lapply(runs, function(RUN) {
      ...
      whichDT <- grepMulti(x = allTbs, patterns = c(
        yr[length(yr)],
        eachClim,
        RUN
      ))
