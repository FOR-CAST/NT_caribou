biomassPlotsCaribou <- function(years = c(2011, 2100),
  ...
  allScenarios <- lapply(Scenarios, function(scenario) {
    ...
    allRuns <- lapply(runs, function(RUN) {
      ...
      bothYears <- lapply(years, function(Y) {
        coh <- bringObjectTSqs(
          path = file.path(pathData, paste(scenario, RUN, sep = "_")),
          rastersNamePattern = c("cohortData", Y)
        )[[1]]
        ras <- bringObjectTSqs(
          path = file.path(pathData, paste(scenario, RUN, sep = "_")),
          rastersNamePattern = c("pixelGroupMap", Y)
        )[[1]]
        ...
        biomassStack <- SpaDES.tools::rasterizeReduced(
          reduced = cohortReduced,
          fullRaster = ras,
  ...
  eachScenarioAverage <- lapply(names(allScenarios), FUN = function(eachScenario) {
    ...
    maxV <- max(
      abs(round(minValue(climateDiffAverage), 1)),
    ...
    climateDiffAverage[flammableRTM == 0] <- NA
    climateDiffAverage <- maskInputs(
      x = climateDiffAverage,
      studyArea = studyArea
    )
    if (!file.exists(biomassDiffPlotPath)) {
      png(
        filename = biomassDiffPlotPath,
        width = 21, height = 29,
        units = "cm", res = 300
      )
  ...
  return(list(
    eachScenario = eachScenarioAverage,
    allScenarios = climateDiffAverage
  ))
}
