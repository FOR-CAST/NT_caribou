Require(c("colorspace", "lattice", "maptools", "rasterVis", "viridis"))
  ...
  if (any(overwriteCalc,
    ...
    booMaps <- lapply(X = climateModels, function(cm){
      runsMaps <- lapply(X = runs, function(RUN){
        ...
        if (all(file.exists(polyRSFfile), file.exists(diffRasName))) {
          ...
          if (length(fRasName) == 0) {
            stop(paste0("File for ", paste0(cm, " ", RUN),
          ...
          raster::writeRaster(x = diffRas, filename = diffRasName, format = "GTiff")
      ...
      if (any(overwriteCalc, !file.exists(paste0(meanDiffName, ".tif")))) {
      ...
      if (any(overwriteCalc, !file.exists(paste0(sdDiffName, ".tif")))) {
    ...
    if (any(overwriteCalcAll, !file.exists(paste0(meanDiffName, ".tif")))) {
    ...
    if (any(overwriteCalcAll, !file.exists(paste0(sdDiffName, ".tif")))) {
