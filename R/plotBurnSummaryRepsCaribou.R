plotBurnSummaryRepsCaribou <- function(dataPath,
  ...
  if (!is.null(theObject)){
    ...
    whichPaths <- allDirs[grepl(x = allDirs,
                                pattern = paste(typeSim, collapse = "|"))]
    allFl <- list.files(whichPaths,
                        ...
                        pattern = paste0("burnSummary_year",
                                         ...
                                         extens),
                        full.names = TRUE)
    funCall <- if (extens == ".qs") qs::qread else
      if (extens == ".rds") readRDS else stop("burnSummary file extension can only be qs or rds")
    burnSumm <- rbindlist(lapply(seq_along(allFl),
                                 FUN = function(fl){
                                   burnTab <- do.call(what = funCall,
                                                      args = list(file = allFl[fl]))
  ...
  ####################### Area burned #######################
  burnSumm[, sumAB := sum(areaBurnedHa), by = c("year", "repetition")]
  ...
  names(Fstats) <- NULL
  pValueA <- ifelse(pf(Fstats[1], Fstats[2], Fstats[3],
                       lower.tail = F) < 0.01,
                    " \n(significant)",
                    " \n(non-significant)")

  areaB[, var := "area_burned"]
  areaB[, val := sumAB/1000]
  ...
  burnSumm[areaBurnedHa > 6.25, fireSize := mean(areaBurnedHa, na.rm = TRUE),
  ...
  if (saveAsQS){
    ...
    write.csv(dt, file.path(dataPath, paste0("burnSummaryTable", paste(typeSim, collapse = "_"),".csv")))
  }
  return(list(fileLocation = fileName, model = list(areaBurnedHa = tend,
                                                    noFires = tendF,
                                                    fireSize = tendS)))
}
